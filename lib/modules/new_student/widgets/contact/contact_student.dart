...
class ContactStudentPage extends StatelessWidget {
  const ContactStudentPage({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
        create: (context) => RegisterCubit()..fetchContacts(), child: const ContactStudentPageView());
  }
}

class ContactStudentPageView extends StatelessWidget {
  const ContactStudentPageView({super.key});
                  ...
                  st.loading ? const LoadingPage() : ListView(
                    ...
                    onTap: () async {
                      if (e.phones.isNotEmpty) {
                        // NewStudentCubit.contactStudent.text = e.phones.first.number.replaceAll(RegExp(r'^([^,.]*[.,])|\D+'), "").replaceAll("62", "0");
                      } else {
