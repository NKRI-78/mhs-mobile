...
class ContactParentPage extends StatelessWidget {
  const ContactParentPage({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
        create: (context) => RegisterCubit()..fetchContacts(), child: const ContactParentPageView());
  }
}

class ContactParentPageView extends StatelessWidget {
  const ContactParentPageView({super.key});
                  ...
                  st.loading ? const LoadingPage() : ListView(
                    ...
                    onTap: () async {
                      if (e.phones.isNotEmpty) {
                        // NewStudentCubit.contactPerent.text = e.phones.first.number.replaceAll(RegExp(r'^([^,.]*[.,])|\D+'), "").replaceAll("62", "0");
                      } else {
